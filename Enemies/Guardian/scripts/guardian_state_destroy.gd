class_name GuardianStateDestroy extends GuardianState

const PICKUP = preload("res://items/item_pickup/item_pickup.tscn")

@export var anim_name : String = "destroy"
@export var knockback_speed : float = 200.0
@export var decelerate_speed : float = 10

@export_category("AI")

@export_category("Items Drop")
@export var drops : Array[ DropData ]

var _damage_position : Vector2
var _direction : Vector2

func init() -> void:
	guardian.guardian_destroyed.connect( _on_guardian_destroyed )
	pass

func enter() -> void:
	guardian.invulnerable = true 
	_direction = guardian.global_position.direction_to( _damage_position )
	guardian.set_direction( _direction )
	guardian.velocity = _direction * -knockback_speed
	guardian.update_animation( anim_name )
	guardian.animation_player.animation_finished.connect( _on_animation_finished )
	disable_hurt_box()
	drop_items()
	pass 

func exit() -> void:
	pass



func process( _delta : float ) -> GuardianState:
	guardian.velocity -= guardian.velocity * decelerate_speed * _delta
	return null

func physics( _delta : float ) -> GuardianState:
	return null

func _on_guardian_destroyed( hurt_box : HurtBox ) -> void:
	_damage_position = hurt_box.global_position
	state_machine.change_state( self )

func _on_animation_finished(_a: String) -> void:
	# Stop further state changes by disabling the state machine logic
	if state_machine:
		state_machine.set_process(false)
		state_machine.set_physics_process(false)
		state_machine.current_state = null

	# Optionally disconnect animation signal to clean up
	if guardian.animation_player.animation_finished.is_connected(_on_animation_finished):
		guardian.animation_player.animation_finished.disconnect(_on_animation_finished)

	# Finally, free the enemy
	guardian.queue_free()

	
func disable_hurt_box() -> void:
	var hurt_box : HurtBox = guardian.get_node_or_null("HurtBox")
	if hurt_box:
		hurt_box.monitoring = false


func drop_items() -> void:
	if drops.size() == 0:
		return
	
	for i in drops.size():
		if drops[ i ] == null or drops[ i ].item == null:
			continue
		var drop_count : int = drops[ i ].get_drop_count()
		for j in drop_count:
			var drop : ItemPickup = PICKUP.instantiate() as ItemPickup
			drop.item_data = drops[ i ].item
			guardian.get_parent().call_deferred( "add_child", drop )
			drop.global_position = guardian.global_position 
			drop.velocity = guardian.velocity.rotated( randf_range( -1.5, 1.5 ) ) * randf_range( 0.9, 1.5 )
	pass
